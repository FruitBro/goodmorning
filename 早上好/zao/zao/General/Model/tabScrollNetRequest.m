...
@implementation tabScrollNetRequest

-(void)getTabScrollRequestInterNet:(NSString *)string{
    
    AFHTTPRequestOperationManager *manager = [[AFHTTPRequestOperationManager alloc]init];
    [manager GET:string parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
        if (self.delegate && [self.delegate respondsToSelector:@selector(configurationTabScrollModelWithRequsetData:)]) {
            [self.delegate configurationTabScrollModelWithRequsetData:responseObject];
        }
